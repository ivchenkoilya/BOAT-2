(()=>{
  ...
  let crack={active:false,safe:null,mode:"",stages:[],stageIndex:0,durability:100,progress:0,turning:false,angle:0,targetAngle:0,tolerance:8,errorCooldown:0,pins:[],needle:0,target:0,direction:1,speed:110,round:0,rounds:3,window:18,failed:false,resolving:false};
  ...
  function pointHitsWall(x,y){
    ...
    const c=Math.floor(x/CELL),r=Math.floor(y/CELL),cell=cellAt(c,r);if(!cell)return true;
    const lx=x-c*CELL,ly=y-r*CELL;
    return (cell.n&&ly<WALL)||(cell.s&&ly>CELL-WALL)||(cell.w&&lx<WALL)||(cell.e&&lx>CELL-WALL);
  }
  ...
  function crackFailed(){
    if(crack.failed)return;crack.failed=true;crack.turning=false;$("crackAbort").disabled=true;
  ...
  function crackStageSuccess(){
    if(crack.failed||crack.resolving)return;crack.resolving=true;crack.turning=false;$("crackAbort").disabled=true;crack.progress=1;$("crackProgressBar").style.width="100%";$("crackMessage").textContent="Механизм поддался!";$("crackMessage").className="crackMessage success";tg?.HapticFeedback?.notificationOccurred?.("success");
    if(crack.stageIndex<crack.stages.length-1){setTimeout(()=>{crack.stageIndex++;crack.progress=0;crack.resolving=false;$("crackAbort").disabled=false;renderCrackStage();},450);return;}
  ...
  function updateCrack(dt){
    if(!crack.active||crack.failed||crack.resolving)return;if(crack.errorCooldown>0)crack.errorCooldown-=dt;
