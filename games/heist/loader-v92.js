(()=>{
  ...
  const boot=async()=>{
    try{
      ...
      script.onerror=async error=>{console.error("Heist v92 transformed game failed",error);URL.revokeObjectURL(url);await loadScript("/games/heist/game.js?v=92");};document.body.appendChild(script);
    }catch(error){console.error("Heist v92 loader failed",error);await loadScript("/games/heist/game.js?v=92");}
  };
  boot();
})();
