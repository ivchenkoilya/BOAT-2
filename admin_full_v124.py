...
VERSION = "Reality 132 · Полный админ-центр власти и биржи"
...
def _full_html() -> str:
    ...
    source = source.replace(
        ...
        "<title>Админ-центр Reality 132</title>",
    )
    source = source.replace(
        '<strong id="versionText">Reality 89</strong>',
        '<strong id="versionText">Reality 132</strong>',
    )
    source = source.replace(
...
def install_admin_full_v124(core: Any) -> None:
    ...
    async def start_server_with_admin_v124(bot: Any):
        if not LEGACY_HTML.is_file() or not FULL_SCRIPT.is_file():
            raise RuntimeError("Не найдены файлы полного админ-центра Reality 132")
