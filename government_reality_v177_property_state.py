...
async def property_state_v177(core: Any, chat_id: int, user_id: int, base: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in base.get("my_items", []):
        ...
        item.update(
            {
                "upgrade_level": level,
                "insurance_until": int(meta.get("insurance_until") or 0),
                "insurance_used": bool(meta.get("insurance_used") or 0),
                "is_primary": bool(meta.get("is_primary") or 0) and str(item.get("status")) == "owned",
    ...
    for person in base.get("declarations", []):
        ...
        for item in person.get("items", []):
            ...
            item["is_primary"] = bool(meta.get("is_primary") or 0) and str(item.get("status")) == "owned"
    ...
    cursor = await conn.execute(
        "SELECT operation_type,amount,detail,property_id,created_at FROM government_property_operations_v176 WHERE chat_id=? ORDER BY created_at DESC LIMIT 50",
        (int(chat_id),),
    )
    ...
    for row in list(await cursor.fetchall()):
        ...
        async with core.db.lock:
            if await (
                ...
                await conn.execute(
                    "UPDATE government_property_v176 SET next_maintenance_at=next_maintenance_at+? WHERE property_id=?",
                    (WEEK, property_id),
                )
                await conn.commit()
                continue
            ...
            cursor2 = await conn.execute(
                ...
                (amount, now, int(row["chat_id"]), int(row["owner_id"]), amount),
            )
            if int(cursor2.rowcount or 0) > 0:
                await conn.execute(
                    "UPDATE government_state_v127 SET treasury=treasury+?,updated_at=? WHERE chat_id=?",
                    (amount, now, int(row["chat_id"])),
                )
                await conn.execute(
                    "INSERT INTO government_property_maintenance_v176(charge_id,property_id,chat_id,owner_id,period_start,amount,paid,status,created_at,paid_at) VALUES(?,?,?,?,?,?,1,'paid_v177',?,?)",
                    (charge_id, property_id, int(row["chat_id"]), int(row["owner_id"]), period_start, amount, now, now),
                )
                await conn.execute(
                    "UPDATE government_property_v176 SET next_maintenance_at=next_maintenance_at+?,updated_at=? WHERE property_id=?",
                    (WEEK, now, property_id),
                )
                await conn.execute(
                    "INSERT INTO score_log(chat_id,user_id,delta,reason,created_at) VALUES(?,?,?,?,?)",
                    (int(row["chat_id"]), int(row["owner_id"]), -amount, "property_maintenance_v177", now),
                )
                await gov._treasury_log(
                    core,
                    int(row["chat_id"]),
                    amount,
                    f"Содержание имущества: {spec['title']}",
                    "property_maintenance_v177",
                    property_id,
                    int(row["owner_id"]),
                )
                await operation(
                    ...
                    int(row["owner_id"]),
                    "maintenance_paid",
                    "Содержание имущества Reality 177",
                    amount=amount,
                ...
                await conn.execute(
                    "INSERT INTO government_property_maintenance_v176(charge_id,property_id,chat_id,owner_id,period_start,amount,paid,status,created_at,paid_at) VALUES(?,?,?,?,?,?,1,'insured_v177',?,?)",
                    (charge_id, property_id, int(row["chat_id"]), int(row["owner_id"]), period_start, amount, now, now),
                )
                await conn.execute(
                    "UPDATE government_property_meta_v177 SET insurance_used=1,updated_at=? WHERE property_id=?",
                    (now, property_id),
                )
                await conn.execute(
                    "UPDATE government_property_v176 SET next_maintenance_at=next_maintenance_at+?,updated_at=? WHERE property_id=?",
                ...
                await operation(
                    ...
                    int(row["owner_id"]),
                    "maintenance_insured",
                    "Страховка покрыла просрочку содержания",
                    amount=amount,
                    property_id=property_id,
                    target_user_id=int(row["owner_id"]),
                )
                message = (
                    "🛡 <b>СТРАХОВКА ИМУЩЕСТВА СРАБОТАЛА</b>\n\n"
                    f"Объект защищён от ареста за просрочку на сумму <b>{fmt(amount)}</b>. Страховка использована."
                )
            else:
                await conn.execute(
                    "INSERT INTO government_property_maintenance_v176(charge_id,property_id,chat_id,owner_id,period_start,amount,paid,status,created_at,paid_at) VALUES(?,?,?,?,?,?,0,'debt_v177',?,0)",
                ...
                await operation(
                    ...
                    int(row["owner_id"]),
                    "maintenance_debt",
                    "Образован долг по содержанию имущества",
                    amount=amount,
                    property_id=property_id,
                    target_user_id=int(row["owner_id"]),
                )
                message = (
                    "🔒 <b>ИМУЩЕСТВО АРЕСТОВАНО ЗА ДОЛГ</b>\n\n"
                    f"Долг за содержание: <b>{fmt(amount)}</b>."
                )
            await conn.commit()
        if message:
