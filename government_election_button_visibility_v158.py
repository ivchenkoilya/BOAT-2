...
VERSION = "Reality 158"
ASSET = Path(__file__).resolve().parent / "governmentapp_v127" / "election-button-visibility-v158.js"


def install_government_election_button_visibility_v158(core: Any) -> None:
    if getattr(core, "_government_election_button_visibility_v158_installed", False):
        return
    core._government_election_button_visibility_v158_installed = True
    ...
    def inject(source: str) -> str:
        ...
        if ASSET.name not in source:
            source = source.replace(
                "</body>",
                f'  <script src="/government-v158/{ASSET.name}?v=158"></script>\n</body>',
            )
        return source
    ...
    async def start(bot: Any):
        if not ASSET.is_file():
            raise RuntimeError("Missing Reality 158 election asset")
        ...
        def runner(app: Any, *args: Any, **kwargs: Any):
            path = f"/government-v158/{ASSET.name}"
            known = {
