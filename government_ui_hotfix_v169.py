...
VERSION = "Reality 169 · Исправление интерфейса Надзора"
...
def install_government_ui_hotfix_v169(core: Any) -> None:
    ...
    def inject_ui_hotfix(source: str) -> str:
        ...
        if ASSET_CSS.name not in source:
            source = source.replace(
                "</head>",
                f'  <link rel="stylesheet" href="/government-v169/{ASSET_CSS.name}?v=169">\n</head>',
            )
        if ASSET_JS.name not in source:
            source = source.replace(
                "</body>",
                f'  <script src="/government-v169/{ASSET_JS.name}?v=169"></script>\n</body>',
            )
        return source
    ...
    async def start_with_ui_hotfix(bot: Any):
        if not ASSET_JS.is_file() or not ASSET_CSS.is_file():
            raise RuntimeError("Не найдены ассеты исправления интерфейса Reality 169")
