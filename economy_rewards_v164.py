...
def install_economy_rewards_v164(core: Any) -> None:
    ...
    def inline_menu_specs_v164(user: Any) -> list[dict[str, Any]]:
        ...
        for item in items:
            ...
            if action == "score:influence":
                item["description"] = (
                    "500–1500 базовых очков · бонусы Древа сверху · кулдаун 6 часов"
                )
            elif action == "fate":
    ...
    # Убираем старые цифры из карточки «О боте» и добавляем новый игровой баланс.
    original_about = core.about_bot_text

    def about_bot_text_v164() -> str:
        text = original_about()
        text = text.replace(
            "базовая награда команды повышена с 1–100 до 50–150 влияния;",
            "базовая награда «Увеличить влияние» составляет 500–1500 влияния;",
        ).replace(
            "50–150 базовых очков",
            "500–1500 базовых очков",
        )
        addition = (
            "\n\n🎲 <b>КУБИК И МОНЕТКА ПРОТИВ БОТА</b>\n"
            "• шанс победы игрока — 45%;\n"
            "• шанс победы бота — 55%;\n"
            "• Древо может уменьшить проигрыш, но не отменить его полностью: минимум −1.\n\n"
            "🔮 <b>УСИЛЕННЫЕ НАГРАДЫ</b>\n"
            "• положительные исходы Шара судьбы увеличены в 10 раз;\n"
            "• положительная награда «Увеличить влияние» увеличена в 10 раз."
        )
        return text if "КУБИК И МОНЕТКА ПРОТИВ БОТА" in text else text + addition

    core.about_bot_text = about_bot_text_v164
