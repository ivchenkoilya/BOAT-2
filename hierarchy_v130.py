...
async def active_user_count(core: Any, chat_id: int) -> int:
    ...
    if count:
        return count
    cursor = await conn.execute(
        "SELECT COUNT(*) amount FROM players WHERE chat_id=?",
        (int(chat_id),),
    )
    row = await cursor.fetchone()
...
def _type_specs(core: Any) -> dict[str, dict[str, str]]:
...
async def _load_maps(core: Any, chat_id: int, now: int) -> dict[str, Any]:
    conn = core.db._require_connection()
    ...
    cursor = await conn.execute(
        """
        SELECT author_id,COUNT(*) amount
        FROM government_bills_v127
        WHERE chat_id=?
        GROUP BY author_id
        """,
        (int(chat_id),),
    ...
    bills = {
        int(row["author_id"]): int(row["amount"])
        for row in await cursor.fetchall()
    }
    ...
    bill_votes = {
        int(row["voter_id"]): int(row["amount"])
        for row in await cursor.fetchall()
    }
    ...
    power_actions = {
        int(row["actor_id"]): int(row["amount"])
        for row in await cursor.fetchall()
    }
    ...
    return {
        "talent_branches": talent_branches,
        "offices_by_user": offices_by_user,
...
async def _state(core: Any, bot: Any, chat_id: int, viewer_id: int) -> dict[str, Any]:
    ...
    type_specs = _type_specs(core)
    maps = await _load_maps(core, chat_id, now)

    cursor = await conn.execute(
        """
        SELECT p.user_id,p.username,p.full_name,p.points,p.career_points,
               p.message_count,p.updated_at,
               COALESCE(tp.total_points,0) talent_total,
               COALESCE(tp.spent_points,0) talent_spent,
               COALESCE(tt.type_key,'') type_key,
               COALESCE(b.messages,p.message_count) behavior_messages,
               COALESCE(b.replies_sent,0) replies_sent,
               COALESCE(b.replies_received,0) replies_received,
               COALESCE(b.reactions_received,0) reactions_received,
               ...
               COALESCE(b.top_checks,0) top_checks,
               COALESCE(b.profile_checks,0) profile_checks,
               COALESCE(b.inline_uses,0) inline_uses,
               COALESCE(b.updated_at,0) behavior_updated_at
        FROM players p
        LEFT JOIN talent_profiles tp
          ON tp.chat_id=p.chat_id AND tp.user_id=p.user_id
        LEFT JOIN today_type_assignments tt
          ON tt.chat_id=p.chat_id AND tt.user_id=p.user_id AND tt.expires_at>?
        LEFT JOIN user_behavior b
          ON b.chat_id=p.chat_id AND b.user_id=p.user_id
        WHERE p.chat_id=?
    ...
    for row in rows:
        user_id = int(row["user_id"])
        branches = maps["talent_branches"].get(
            user_id,
            {"damage": 0, "influence": 0, "defense": 0, "rewards": 0},
        )
        offices = maps["offices_by_user"].get(user_id, [])
        game = maps["game_stats"].get(
            user_id,
            {"won": 0, "lost": 0, "wins": 0, "losses": 0},
        )
        messages = max(
            int(row["message_count"] or 0),
            int(row["behavior_messages"] or 0),
        )
        last_activity = max(
            int(row["updated_at"] or 0),
            int(row["behavior_updated_at"] or 0),
        )
        ...
        type_spec = type_specs.get(type_key, {})
        activity_score = (
            messages
        ...
        government_score = (
            len(offices) * 40
            + maps["bills"].get(user_id, 0) * 12
            + maps["bill_votes"].get(user_id, 0) * 4
            + maps["power_actions"].get(user_id, 0) * 8
        )
        aura = max(
            1,
            min(
                1000,
                100
                + int(row["reactions_received"] or 0) * 8
                + int(row["replies_received"] or 0) * 4
                + int(row["positive_actions"] or 0) * 5,
            ),
        )
        chsv = max(
            1,
            min(
                1000,
                100
                + int(row["top_checks"] or 0) * 5
                + int(row["profile_checks"] or 0) * 3
                + int(row["inline_uses"] or 0)
                + int(row["positive_actions"] or 0) * 2,
            ),
        )
        participants.append(
            {
                ...
                "talent_available": max(
                    0,
                    int(row["talent_total"] or 0)
                    - int(row["talent_spent"] or 0),
                ),
                "talent_branches": branches,
                "offices": offices,
                "government_actions": maps["power_actions"].get(user_id, 0),
                "bills_authored": maps["bills"].get(user_id, 0),
                "bill_votes": maps["bill_votes"].get(user_id, 0),
        ...
        item["power_score"] = int(
            round(
                ...
                * (
                    ...
                    + 0.10
                    * _norm(item["government_score"], maxima["government"])
                    + 0.10 * _norm(item["activity_score"], maxima["activity"])
                    + 0.05
                    * _norm(max(0, item["game_profit"]), maxima["games"])
                )
            )
        )
    ...
    comparison_date = datetime.fromtimestamp(
        now - 6 * 86400,
        timezone.utc,
    ).date().isoformat()
    for rank, item in enumerate(participants, 1):
        ...
        cursor = await conn.execute(
            """
            SELECT rank
            FROM hierarchy_rank_snapshots_v130
            WHERE chat_id=? AND user_id=? AND date_key<=?
            ORDER BY date_key DESC
            LIMIT 1
            """,
            (int(chat_id), int(item["user_id"]), comparison_date),
        )
    ...
    government_rows = list(maps["government_rows"])
    government_rows.sort(
        key=lambda item: (
            OFFICE_ORDER.get(str(item["office_key"]), 99),
            int(item["seat_no"]),
        )
    )
    ...
    return {
        ...
        "chat": {
            "chat_id": int(chat_id),
            "title": await government._chat_title(bot, chat_id),
        },
...
def install_hierarchy_v130(core: Any) -> None:
    ...
    async def state_api(request: Any):
        try:
            ...
            core.logging.exception(
                "Ошибка Иерархии реальности Reality 130"
            )
            return core.web.json_response(
                {"ok": False, "reason": str(exc)},
                status=400,
    ...
    async def start_with_hierarchy(bot: Any):
        if not APP_DIR.is_dir():
            raise RuntimeError(
                f"Не найдена папка Иерархии Reality 130: {APP_DIR}"
            )
        ...
        def runner_factory(app: Any, *args: Any, **kwargs: Any):
            ...
            if ("GET", "/hierarchy-v130/") not in keys:
                app.router.add_get("/hierarchy-v130/", index)
            app.router.add_get(
                "/hierarchy-v130/{name:app\\.css|app\\.js}",
                asset,
            )
            if ("GET", "/hierarchy-v130/api/state") not in keys:
                app.router.add_get(
                    "/hierarchy-v130/api/state",
                    state_api,
                )
    ...
    def commands_with_hierarchy() -> list[BotCommand]:
        commands = [
            item
            for item in original_commands()
            if item.command != "hierarchy"
        ]
        position = next(
            (
                index + 1
                for index, item in enumerate(commands)
                if item.command == "government"
            ),
            len(commands),
        )
        commands.insert(
    ...
    handlers[:] = [
        ...
        if getattr(handler.callback, "__name__", "")
        != "cmd_hierarchy_v130"
    ]

    @core.router.message(
        Command("hierarchy", "ratings", "participants")
    )
    async def cmd_hierarchy_v130(message: Message) -> None:
        ...
        if not link:
            await message.answer(
                "⚠️ Адрес Mini App «Иерархия реальности» не настроен."
            )
            return
        ...
        await message.answer(
            ...
            "Общий топ, паспорта участников, карьерные звания, типажи, "
            "Древо талантов, игры, активность и государственные должности "
            "в одном приложении.\n\n"
            f"Сейчас активно: <b>{active}</b>. Рекомендуемый состав "
            f"Госдумы: <b>{deputies} депутата</b>.",
