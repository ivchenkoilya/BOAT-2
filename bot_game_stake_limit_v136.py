...
def install_bot_game_stake_limit_v136(core: Any) -> None:
    if getattr(core, "_bot_game_stake_limit_v136_installed", False):
        return
    core._bot_game_stake_limit_v136_installed = True
    core.BOT_GAME_STAKE_VERSION = VERSION
    core.BOT_GAME_MAX_STAKE = MAX_BOT_GAME_STAKE

    original_parser = core.parse_bot_game_query

    def parse_bot_game_query_v136(query: str):
        parsed = _parse_game_and_amount(query)
        return parsed if parsed is not None else original_parser(query)

    core.parse_bot_game_query = parse_bot_game_query_v136

    original_prepare = core.prepare_bot_game_result

    async def prepare_bot_game_result_v136(
        user: Any,
        game_type: str,
        stake: int | None,
        chat_id: int,
    ...
    core.prepare_bot_game_result = prepare_bot_game_result_v136

    original_create = core.Database.create_bot_game

    async def create_bot_game_v136(self: Any, *args: Any, **kwargs: Any):
    ...
    core.Database.create_bot_game = create_bot_game_v136
