...
async def sell_to_state(
    ...
    async with core.db.lock:
        # The idempotency key is checked again inside the financial lock. A delayed
        # duplicate request therefore receives the first result instead of an error.
        if request_id:
            cursor = await conn.execute(
                "SELECT result_text FROM government_property_action_requests_v177 WHERE request_id=?",
                (request_id,),
        ...
        state_owner_id = -max(
            1,
            int.from_bytes(
                hashlib.sha256(str(property_id).encode("utf-8")).digest()[:8],
                "big",
            ),
        )
        cursor = await conn.execute(
        ...
        await conn.execute(
            ...
            (payout, now, int(chat_id)),
        )
        await conn.execute(
            "UPDATE players SET points=points+?,updated_at=? WHERE chat_id=? AND user_id=?",
            (payout, now, int(chat_id), int(actor_id)),
        )
        await conn.execute(
            "INSERT INTO score_log(chat_id,user_id,delta,reason,created_at) VALUES(?,?,?,?,?)",
            (int(chat_id), int(actor_id), payout, "property_sale_to_state_v177", now),
        )
        await gov._treasury_log(
            core,
            int(chat_id),
            -payout,
            "Государственный выкуп имущества",
            "property_state_buyout_v177",
            str(property_id),
            int(actor_id),
        )
        await operation(
            ...
            chat_id,
            actor_id,
            "property_sale_to_state",
            "Продажа имущества государству",
            amount=payout,
    ...
    await gov._publish(
        bot,
        int(chat_id),
        "💰 <b>ГОСУДАРСТВО ВЫКУПИЛО ИМУЩЕСТВО</b>\n\n"
        f"Владелец получил <b>{fmt(payout)}</b> влияния. Объект перешёл в государственную собственность.",
    )
    return result
...
async def upgrade_property(core: Any, chat_id: int, actor_id: int, property_id: str) -> str:
    ...
    async with core.db.lock:
        ...
        if reason:
            raise ValueError(reason)
        cursor = await conn.execute(
            "SELECT upgrade_level FROM government_property_meta_v177 WHERE property_id=?",
            (str(property_id),),
        )
        ...
        await conn.execute(
            "INSERT INTO score_log(chat_id,user_id,delta,reason,created_at) VALUES(?,?,?,?,?)",
            (int(chat_id), int(actor_id), -cost, "property_upgrade_v177", now),
        )
        await gov._treasury_log(
            core,
            int(chat_id),
            cost,
            f"Улучшение имущества до уровня {level + 1}",
            "property_upgrade_fee_v177",
            str(property_id),
            int(actor_id),
        )
        await operation(
...
async def insure_property(core: Any, chat_id: int, actor_id: int, property_id: str) -> str:
    ...
    async with core.db.lock:
        ...
        cursor = await conn.execute(
            "SELECT insurance_until,insurance_used FROM government_property_meta_v177 WHERE property_id=?",
            (str(property_id),),
        )
        meta = await cursor.fetchone()
        ...
        await conn.execute(
            "INSERT INTO score_log(chat_id,user_id,delta,reason,created_at) VALUES(?,?,?,?,?)",
            (int(chat_id), int(actor_id), -cost, "property_insurance_v177", now),
        )
        await gov._treasury_log(
            core,
            int(chat_id),
            cost,
            "Страхование имущества на 7 дней",
            "property_insurance_fee_v177",
            str(property_id),
            int(actor_id),
        )
        await operation(
